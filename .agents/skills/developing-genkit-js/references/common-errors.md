...
- **Correct (v1.x)**: Do NOT await `generateStream`. Iterate over `stream` directly. Await `response` property for final result. Access properties via `chunk.text` or `chunk.content`.
  ```ts
  const {stream, response} = ai.generateStream(...); // NO await here
  for await (const chunk of stream) {
    console.log(chunk.text);                         // CORRECT (property access)
  }
  const finalResponse = await response;              // Await response property
  ```
- **Incorrect (Pre-1.0)**: Calling stream as a function, awaiting the generator incorrectly, or calling chunk properties as functions.
  ```ts
  for await (const chunk of stream()) { ... } // INCORRECT
  await response();                           // INCORRECT
  console.log(chunk.text());                  // INCORRECT (method call)
  ```
